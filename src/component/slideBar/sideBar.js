...
export default function AdminLayOut({children}) {
  ...
  return (
    <Box sx={{ display: 'flex' }} className={Style.SideBar}>
      ...
      <AppBar position="fixed" open={open} className={styles.headerBar}>
        <Toolbar className={styles.Headerlayout}>
      ...
      <Drawer variant="permanent" open={open} className={styles.sidleBarMenu}>   
        ...
        <List>
          {Menus.map((menu, index) => (
            <ListItem key={menu.path} disablePadding sx={{ display: 'block' }} onClick={()=> navigate(menu.path)} >
              <ListItemButton
                sx={{
                  minHeight: 48,
